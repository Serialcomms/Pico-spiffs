...
char stderr_printf_buffer[80];

spiffs_flags RAM_ONLY_FUNCTION (fopen_flags_to_spiffs_open_flags)(uint32_t fopen_flags);
...
int RAM_ONLY_FUNCTION (_open)(const char *fn, int oflag, ...) {
    ...
    spiffs_file file_handle = SPIFFS_open(&pico_fs, fn, spiffs_open_flags, 0);

    if (file_handle < 100) {  // should be a +ve number starting from 100 as per spiffs fh_ix_offset = 99
...
int RAM_ONLY_FUNCTION (_lseek)(int fd, off_t pos, int whence) {
    ...
    if (_is_a_tty(fd)) {
        ...
        seek_result = EISDIR; // not a file

    } else {

        seek_result = SPIFFS_lseek(&pico_fs, fd, pos, whence);
...
int RAM_ONLY_FUNCTION (_read)(int handle, char *buffer, int length) {
    ...
    if (handle == STDIO_HANDLE_STDIN) {
        ...
        read_result = _read_from_tty(buffer, length);

    } else {

        read_result = SPIFFS_read(&pico_fs, handle, buffer, length);
...
int RAM_ONLY_FUNCTION (_write)(int handle, char *buffer, int length) {
    ...
    if (handle == STDIO_HANDLE_STDOUT || handle == STDIO_HANDLE_STDERR) {
        ...
        write_result = _write_to_tty(buffer, length);

    } else {

        write_result = SPIFFS_write(&pico_fs, handle, buffer, length);
...
int RAM_ONLY_FUNCTION (_close)(int fd) {
    ...
    if (_is_a_tty(fd)) {
        ...
        close_result = EISDIR; // not a file

    } else {

        close_result = SPIFFS_close(&pico_fs,fd);
...
int RAM_ONLY_FUNCTION (_fstat)(int fd, struct stat *buf) {
    ...
    if (_is_a_tty(fd)) {
        ...
        fstat_result = EISDIR; // not a file

    } else {

        fstat_result = SPIFFS_fstat(&pico_fs, fd, &s);
...
int RAM_ONLY_FUNCTION(_read_from_tty)(char *buffer, int length) {
    ...
    do {
        ...
        //tty_char = stdio_getchar_timeout_us(1000 * 1000);

        tty_char = stdio_getchar();

        if (tty_char >= 0) {

            buffer[buffer_position] = tty_char & 0xff;

            ++buffer_position;

        }
 
    } while (buffer_position < length && tty_char >= 0);
...
int RAM_ONLY_FUNCTION(_write_to_tty)(char *buffer, int length) {
    ...
    do {
        ...
        uint8_t tty_char = buffer[buffer_position];
        
        stdio_putchar(tty_char & 0x7F);
...
spiffs_flags RAM_ONLY_FUNCTION (fopen_flags_to_spiffs_open_flags)(uint32_t fopen_flags) {
    ...
    uint32_t fopen_mode = fopen_flags & (O_RDWR | O_WRONLY | O_RDONLY);

    if (fopen_flags > 0x07FF) {

        //dma_printf("Warning - masking fopen flags > 0x7FF %08x\r\n", fopen_flags);

        fopen_flags &= 0x7FF;

    }

    if (fopen_mode == O_RDWR) {

        //dma_printf("fopen file read/write\r\n");

        spiffs_open_flags |= SPIFFS_O_RDWR;         //  0x08
      
    } else if (fopen_mode == O_WRONLY) {
            
        //dma_printf("fopen file write-only\r\n");
        
        spiffs_open_flags |= SPIFFS_O_WRONLY;       //  0x10
          
    } else if (fopen_mode == O_RDONLY) {

        //dma_printf("fopen file read-only\r\n");
        
        spiffs_open_flags |= SPIFFS_O_RDONLY;       //  0x18
    
    } else {

        //dma_printf("fopen error, fopen flags = %04x\r\n", fopen_flags);

    }

    if (fopen_mode == O_RDONLY)  {

        if ((fopen_flags & O_EXCL)    == O_EXCL)   spiffs_open_flags |= SPIFFS_O_EXCL;     // 0x40

    } else {

        if ((fopen_flags & O_APPEND)  == O_APPEND) spiffs_open_flags |= SPIFFS_O_APPEND;   // 0x01
        if ((fopen_flags & O_TRUNC)   == O_TRUNC)  spiffs_open_flags |= SPIFFS_O_TRUNC;    // 0x02
        if ((fopen_flags & O_CREAT)   == O_CREAT)  spiffs_open_flags |= SPIFFS_O_CREAT;    // 0x04
        if ((fopen_flags & O_EXCL)    == O_EXCL)   spiffs_open_flags |= SPIFFS_O_EXCL;     // 0x40
