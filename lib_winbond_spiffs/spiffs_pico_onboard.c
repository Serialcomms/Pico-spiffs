...
#include <spiffs_pico_onboard.h>

#define ONBOARD_LOG_PAGE_SIZE 256
#define ONBOARD_FLASH_PAGE_SIZE 256U
  
u8_t PICO_RAM_ONLY_ALIGNED_DATA spiffs_onboard_fds[32*4];
u8_t PICO_RAM_ONLY_ALIGNED_DATA spiffs_onboard_work_buf[ONBOARD_LOG_PAGE_SIZE*4];
u8_t PICO_RAM_ONLY_ALIGNED_DATA spiffs_onboard_cache_buf[(ONBOARD_LOG_PAGE_SIZE+32)*32];

spiffs pico_fs;

spiffs_config pico_spiffs_onboard_config;

void spiffs_list_directory();

void onboard_test_posix();

static s32_t RAM_ONLY_FUNCTION(pico_spiffs_onboard_read)(u32_t addr, u32_t size, u8_t *dst) {
  ...
  memcpy(dst, xip_flash_address, size);
...
static s32_t RAM_ONLY_FUNCTION(pico_spiffs_onboard_write_part_page)(u32_t addr, u32_t size, u8_t *src) {
  ...
  memset(page_reflash_buffer, (uint8_t)0xFF, ONBOARD_FLASH_PAGE_SIZE);

  memcpy(reflash_buffer, src, size);
...
static s32_t RAM_ONLY_FUNCTION(pico_spiffs_onboard_write)(u32_t addr, u32_t size, u8_t *src) {
  ...
  // As a consequence of how NOR flash works, any bytes written with value 0xFF will
  // not change any existing flash data when programmed.
  ...
  if (address_page_boundary != 0 || size != ONBOARD_FLASH_PAGE_SIZE) {
    ...
    pico_spiffs_onboard_write_part_page(addr, size, src);
...
static s32_t RAM_ONLY_FUNCTION(pico_spiffs_onboard_erase)(u32_t addr, u32_t size) {
...
void pico_spiffs_mount_onboard_filesystem() {

  pico_spiffs_onboard_config.hal_read_f = pico_spiffs_onboard_read;
  pico_spiffs_onboard_config.hal_write_f = pico_spiffs_onboard_write;
  pico_spiffs_onboard_config.hal_erase_f = pico_spiffs_onboard_erase;

  pico_spiffs_onboard_config.phys_size = (4 * 1024 * 1024);

  pico_spiffs_onboard_config.phys_addr = 0x0270000;           // start address in onboard Pico flash

  pico_spiffs_onboard_config.log_page_size = 256;

  pico_spiffs_onboard_config.log_block_size = (256 * 256);    // 65k

  pico_spiffs_onboard_config.phys_erase_block = (256 * 256);  // 65k

  pico_spiffs_onboard_config.fh_ix_offset = 99;               // first file handle issued = 100

  printf("Attempting to mount SPIFFS filesystem using Pico onboard flash...\n\n");
  ...
  int mount_result = SPIFFS_mount(&pico_fs,
    ...
    &pico_spiffs_onboard_config,
    spiffs_onboard_work_buf,
    spiffs_onboard_fds,
    sizeof(spiffs_onboard_fds),
    spiffs_onboard_cache_buf,
    sizeof(spiffs_onboard_cache_buf),
    0);
